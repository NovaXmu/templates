...
<html lang=zh>
	...
	<body class="exchange">
				...
				{if is_array($list)}
					...
					<div class="item row">
						...
						<div class="col-xs-8 desc">
							...
							<p>所需网薪：{$value.price|default:'未知价格'}</p>
							...
							<p><a onclick="toggleCondition()" class="toggleCond">限制条件>></a>
								...
								{if array_key_exists("registTime",$value.limitConds)}
									...
									早于{abs($value.limitConds["registTime"])|date_format:'%Y-%m-%d %H:%M'}
									{elseif $value.limitConds["registTime"] > 0}
									晚于{$value.limitConds["registTime"]|date_format:'%Y-%m-%d %H:%M'}
							...
							<div class="row">
								...
								<div class="col-xs-6 col-xs-offset-6">
									<a href="exchange?m=exchange1&id={$value.id|default:'null'}"><img src="" class="exchangeBtn"/></a>
								</div>
							</div>
						</div>
					</div>
		...
		<script src="{$base}mall/js/exchangelist.js"></script>
	</body>
</html>